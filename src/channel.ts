...
import type { ChannelPlugin, ClawdbotConfig, PluginRuntime } from "clawdbot/plugin-sdk";
import { getChatChannelMeta } from "clawdbot/plugin-sdk";
...
function getLarkConfig(cfg: ClawdbotConfig): Record<string, LarkAccountConfig> {
  return (cfg as any).channels?.lark ?? {};
}

function listLarkAccountIds(cfg: ClawdbotConfig): string[] {
...
async function sendLarkMedia(
  ...
  text: string,
  mediaUrl: string,
  options?: { accountId?: string; replyToId?: string; cfg?: ClawdbotConfig }
): Promise<{ messageId: string; to: string }> {
  // 暂时用文本 + 链接代替图片上传
...
export const larkPlugin: ChannelPlugin<ResolvedLarkAccount> = {
  ...
  security: {
    resolveDmPolicy: ({ account }) => ({
      ...
      approveHint: "clawdbot pairing approve lark <code>",
